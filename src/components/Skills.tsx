import { motion } from "framer-motion";
import { useInView } from "framer-motion";
import { useRef } from "react";
import {
  Smartphone,
  Database,
  Cloud,
  Layers,
  Box,
  Flame,
  HardDrive,
  Sparkles,
  Server,
  Users,
  Workflow,
  Package,
  Braces,
  CloudCog,
  Globe,
  SaveAll,
  RefreshCw,
  Search,
} from "lucide-react";

const Skills = () => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true });

  // Icon mapping for skills - using SVG for Flutter, Dart, and Git logos
  const skillIcons = {
    Flutter: "flutter",
    Dart: "dart",
    Firebase: Flame,
    Firestore: Database,
    "Node.js": Server,
    GCP: Cloud,
    "REST API": Globe,
    "API Integration": Workflow,
    Provider: RefreshCw,
    MVVM: Layers,
    GetX: RefreshCw,
    MVC: Layers,
    "Cloud Functions": CloudCog,
    JavaScript: Braces,
    Android: Smartphone,
    NoSQL: Database,
    "CRM Development": Users,
    "Local Storage": SaveAll,
    Git: "git",
    "AI-Assisted Coding": Sparkles,
    "Algolia Search": Search,
  };

  const skillCategories = [
    {
      title: "Technical Skills",
      skills: [
        "Flutter",
        "Dart",
        "Firebase",
        "Firestore",
        "Node.js",
        "GCP",
        "REST API",
        "API Integration",
        "Provider",
        "MVVM",
        "GetX",
        "MVC",
        "Cloud Functions",
        "JavaScript",
        "Android",
        "NoSQL",
        "CRM Development",
        "Local Storage",
        "Git",
        "AI-Assisted Coding",
        "Algolia Search",
      ],
    },
  ];

  return (
    <section id="skills" className="py-20 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-background via-card to-background opacity-50" />

      <div className="container mx-auto px-4 relative z-10" ref={ref}>
        <motion.div
          initial={{ opacity: 0, y: 50 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }}
          className="text-center mb-16"
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4 gradient-text">
            Skills & Expertise
          </h2>
          <div className="w-20 h-1 bg-gradient-to-r from-primary to-secondary mx-auto" />
        </motion.div>

        <div className="max-w-6xl mx-auto space-y-12">
          {skillCategories.map((category, categoryIndex) => (
            <motion.div
              key={categoryIndex}
              initial={{ opacity: 0, y: 30 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: categoryIndex * 0.2 }}
            >
              <h3 className="text-2xl font-bold mb-6 text-foreground">
                {category.title}
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {category.skills.map((skill, skillIndex) => {
                  const iconValue = skillIcons[skill] || Box;
                  const isCustomIcon = typeof iconValue === "string";
                  
                  return (
                    <motion.div
                      key={skillIndex}
                      initial={{ opacity: 0, scale: 0.8 }}
                      animate={isInView ? { opacity: 1, scale: 1 } : {}}
                      transition={{
                        duration: 0.4,
                        delay: categoryIndex * 0.2 + skillIndex * 0.05,
                      }}
                      whileHover={{ scale: 1.1, y: -5 }}
                      className="glass-morphism p-4 rounded-lg text-center neon-border-pink hover:neon-glow-pink transition-all cursor-pointer group"
                    >
                      {isCustomIcon ? (
                        <div className="w-6 h-6 mx-auto mb-2 flex items-center justify-center">
                          {iconValue === "flutter" && (
                            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-primary group-hover:fill-secondary transition-colors">
                              <path d="M14.314 0L2.3 12 6 15.7 21.684.013h-7.357zm.014 11.072L7.857 17.53l6.47 6.47H21.7l-6.46-6.468 6.46-6.46h-7.37z"/>
                            </svg>
                          )}
                          {iconValue === "dart" && (
                            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-primary group-hover:fill-secondary transition-colors">
                              <path d="M4.105 4.105S9.158 1.58 11.684.316a3.079 3.079 0 0 1 1.481-.315c.766.047 1.677.788 1.677.788L24 9.948v9.789h-4.263V24H9.789l-9-9C.303 14.5 0 13.795 0 13.105c0-.319.18-.818.316-1.105l3.789-7.895zm.679.679v11.787c.002.543.021 1.024.498 1.508L10.204 23h8.533v-4.263h4.263V10.79L13.42 1.209l-.004-.005-.588-.588c-.024-.024-.048-.047-.073-.067a1.015 1.015 0 0 0-.438-.183 1.959 1.959 0 0 0-.833.021c-.316.099-.639.263-.95.458-.025.016-.049.032-.073.05L4.784 4.784zm12.055-.678c-.899-.896-2.355-.896-3.256 0-.899.899-.899 2.354 0 3.256.9.896 2.355.896 3.256 0 .899-.9.899-2.354 0-3.256z"/>
                            </svg>
                          )}
                          {iconValue === "git" && (
                            <svg viewBox="0 0 24 24" className="w-6 h-6 fill-primary group-hover:fill-secondary transition-colors">
                              <path d="M23.546 10.93L13.067.452c-.604-.603-1.582-.603-2.188 0L8.708 2.627l2.76 2.76c.645-.215 1.379-.07 1.889.441.516.515.658 1.258.438 1.9l2.658 2.66c.645-.223 1.387-.078 1.9.435.721.72.721 1.884 0 2.604-.719.719-1.881.719-2.6 0-.539-.541-.674-1.337-.404-1.996L12.86 8.955v6.525c.176.086.342.203.488.348.713.721.713 1.883 0 2.6-.719.721-1.889.721-2.609 0-.719-.719-.719-1.879 0-2.598.182-.18.387-.316.605-.406V8.835c-.217-.091-.424-.222-.6-.401-.545-.545-.676-1.342-.396-2.009L7.636 3.7.45 10.881c-.6.605-.6 1.584 0 2.189l10.48 10.477c.604.604 1.582.604 2.186 0l10.43-10.43c.605-.603.605-1.582 0-2.187"/>
                            </svg>
                          )}
                        </div>
                      ) : (
                        (() => {
                          const Icon = iconValue;
                          return <Icon className="w-6 h-6 mx-auto mb-2 text-primary group-hover:text-secondary transition-colors" />;
                        })()
                      )}
                      <span className="text-foreground group-hover:text-primary transition-colors font-medium text-sm">
                        {skill}
                      </span>
                    </motion.div>
                  );
                })}
              </div>
            </motion.div>
          ))}
        </div>

        {/* Animated background elements */}
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(10)].map((_, i) => (
            <motion.div
              key={i}
              className="absolute w-2 h-2 bg-primary rounded-full opacity-20"
              style={{
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
              }}
              animate={{
                y: [0, -30, 0],
                opacity: [0.2, 0.5, 0.2],
              }}
              transition={{
                duration: 3 + Math.random() * 2,
                repeat: Infinity,
                delay: Math.random() * 2,
              }}
            />
          ))}
        </div>
      </div>
    </section>
  );
};

export default Skills;